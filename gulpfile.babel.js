...
gulp.task('clean:styleguide:css', () => {
  return del(['_template-kss/public/**/*.css']);
});
...
// 'gulp styleguide' -- builds your KSS Styleguide 
gulp.task('styleguide', done => {
  shell.exec('node_modules/.bin/kss-node --source src/assets/scss --destination dist/styleguide --template _template-kss');
  done();
});

// 'gulp styleguide-less' creates a CSS file from your LESS Styleguide file.
gulp.task('styleguide-less', () =>
  gulp.src('_template-kss/public/**/*.less')
    .pipe($.less())
    .pipe(gulp.dest('_template-kss/public'))
);
...
// Combine svg files and inject it into the Styleguide
gulp.task('svg-styleguide', () => {
    var svgs = gulp
        .src('src/assets/images/svg/*.svg')
        .pipe($.svgmin())
        .pipe($.svgstore({ inlineSvg: true }));
...
gulp.task('serve', () => {
  ...
  gulp.watch(['src/**/*.md', 'src/**/*.html', 'src/**/*.yml', 'src/**/*.xml', 'src/**/*.txt'], gulp.series('clean:styleguide', 'jekyll', 'styleguide', reload));
...
gulp.task('assets', gulp.series(
  gulp.series('clean:assets', 'clean:styleguide'),
  gulp.series('es6'),
  gulp.parallel('styles', 'scripts', 'fonts', 'images', 'svg', 'svg-styleguide')
));
...
gulp.task('clean', gulp.series('clean:assets', 'clean:images', 'clean:gzip', 'clean:styleguide'));
...
'clean:images', 'clean:metadata', 'clean:styleguide'));
